...
static InfluxDBClient influxClient(INFLUXDB_URL, INFLUXDB_ORG, INFLUXDB_BUCKET, INFLUXDB_TOKEN);
static Point influxPoint("ikea_sensor");
static bool influxReady = false;
...
void loop()
{
  ...
  else
  {
    ...
    if (influxReady)
    {
      ...
      influxPoint.addField("pm2.5", pm2_5);
      influxPoint.addField("co2", co2);
      influxPoint.addField("temp", temperature);
      influxPoint.addField("humidity", humidity);

      // Write point
      if (!influxClient.writePoint(influxPoint))
      {
